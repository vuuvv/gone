...
func TestRedBlackTreePut(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%d%d%d%d%d%d%d", params...), "1234567"; actualValue != expectedValue {
	...
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s%s%s%s", params...), "abcdefg"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
...
func TestRedBlackTreeRemove(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%d%d%d%d", params...), "1234"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	params = []any{}
	for _, k := range tree.Values() {
	...
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s", params...), "abcd"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	params = []any{}
	for _, k := range tree.Values() {
...
func benchmarkGet[V any](b *testing.B, tree *Tree[int, V], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			tree.Get(n)
		}
...
func benchmarkRemove[V any](b *testing.B, tree *Tree[int, V], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			tree.Remove(n)
		}
